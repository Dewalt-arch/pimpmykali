...
fix_missing () { 
     apt -y update 
     echo -e "\n \e[1;33m[+]\e[0m apt updated "
     apt -y remove kali-undercover 
     echo -e "\n \e[1;33m[+]\e[0m kali-undercover removed"
     apt -y install python-pip python3-pip seclists
     echo -e "\n \e[1;33m[+]\e[0m python-pip python3-pip seclists installed "
     } 
...
make_rootgreatagain () {
     ...
     case $userinput in
         ...
         n|N) echo -e "\n \e[1;31m[!]\e[0m skipping root login setup" ;;
...
enable_rootlogin () {
    ...
    passwd root
    echo -e "\n \e[1;33m[+]\e[0m root login enabled \n"
    }    
...
fix_smbconf () {
    ...
    if [ $check_min -ne 0 ] && [ $check_max -ne 0 ]
      then
        echo -e "\n \e[1;31m[-]\e[0m client min protocol is already set not changing\n \e[1;31m[-]\e[0m client max protocol is already set not changing\n\n"
      else
        cat /etc/samba/smb.conf | sed 's/\[global\]/\[global\]\n   client min protocol = CORE\n   client max protocol = SMB3\n''/' > /tmp/fix_smbconf.tmp
        cat /tmp/fix_smbconf.tmp > /etc/samba/smb.conf
        rm -f /tmp/fix_smbconf.tmp
        echo -e "\n \e[1;33m[+]\e[0m /etc/samba/smb.conf updated"
    fi
    }

fix_impacket () { 
    apt -y remove impacket 2> /dev/null
    apt -y install python-pip python3-pip 2> /dev/null 
    ...
    for impacket_file in ${arr[@]}; do
        ...
        # echo -e "\n \e[1;33m[+]\e[0m impacket removed "
	done 
    
    wget https://github.com/SecureAuthCorp/impacket/releases/download/impacket_0_9_19/impacket-0.9.19.tar.gz -O /tmp/impacket-0.9.19.tar.gz 2> /dev/null
    ...
    echo -e "\n \e[1;33m[+]\e[0m python-pip and python3-pip installed" 
    echo -e " \e[1;33m[+]\e[0m python wheel installed"
    echo -e " \e[1;33m[+]\e[0m impacket installed \n" 
    }
...
fix_golang () {
    ...
    echo -e "\n \e[1;33m[+]\e[0m golang installed"
    echo -e "\n \e[1;33m[+]\e[0m .bashrc in /home/kali/.bashrc and /root/.bashrc updated \n"
    }
...
fix_grub () {
    ...
    if [ $check_grub -ne 1 ]
      then 
        echo -e '\n \e[1;31m[!]\e[0m Error: /etc/default/grub is not the default config - not changing'
      else
        ...
        update-grub
	echo -e "\n \e[1;33m[+]\e[0m Added mitigations=off to GRUB_CMDLINE_LINUX_DEFAULT"
	echo -e "\n \e[1;31m[!]\e[0m Reboot for changes to take effect \n"
    fi
    } 
